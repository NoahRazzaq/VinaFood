        ...
        <form method="post" action="{{ route('product.update', $product->id) }}" class="space-y-10 w-full max-w-md" enctype="multipart/form-data">
            ...
            <div class="mb-6">
                ...
                <input type="text" id="detail" name="detail"
                    value="{{ old('detail') ? old('detail') : $product->detail }}"
                    class="border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5  dark:border-gray-600 dark:placeholder-gray-400  dark:focus:ring-blue-500 dark:focus:border-blue-500"
                    placeholder="sandwich poulet">
                    @error('detail')
                    <div class="flex items-center p-2 mb-4 text-sm text-red-800 rounded-lg bg-red-50 dark:text-red-400"
            ...
            <div class="mb-6">
                <label for="email"
                    class="block mb-2 text-sm font-medium text-gray-900 ">Restaurant</label>
                <select name="restaurant"
                    ...
                    <option disabled value="">---Séléctionnez un restaurant---</option>
                    @foreach ($restaurants as $restaurant)
                        <option value="{{ $restaurant->id }}" @if (old('restaurant') == $restaurant->id || $product->restaurant_id == $restaurant->id) selected @endif>
                            {{ $restaurant->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="mb-6">
                <label for="email"
                    class="block mb-2 text-sm font-medium text-gray-900 ">Catégorie</label>
                <select name="category"
                    class="form-select border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5  dark:border-gray-600 dark:placeholder-gray-400  dark:focus:ring-blue-500 dark:focus:border-blue-500">
                    <option selected disabled value=""></option>
                    <option value="">---Séléctionnez une catégorie---</option>
                    @foreach ($categories as $category)
                        <option value="{{ $category->id }}" @if (old('category') == $category->id || $product->category_id == $category->id) selected @endif>
